...
|-----|------|------|----------------|
| `wpconnect-agent/` | WordPress plugin (the gatekeeper that runs on the user's site) | PHP, single file | `Version:` header in `wpconnect-agent.php` |
| `wpconnect-mcp/` | Local MCP client (stdio server the AI client spawns) | Node ESM | `version` in `package.json` |
...
# MCP client (wpconnect-mcp/) — run directly, no build step
node wpconnect-mcp/index.js add-site --site "https://example.com" --token "wpconnect_tk_..." [--default]
node wpconnect-mcp/index.js list-sites
node wpconnect-mcp/index.js remove-site --site "https://example.com"
node wpconnect-mcp/index.js set-default --site "https://example.com"
...
#   zip the wpconnect-agent/ folder into wpconnect-agent.zip for upload via WP admin.
#   (wpconnect-agent.zip is gitignored.)
```
...
- **`wpconnect-mcp` config** lives at `~/.wpconnect.json`: `{ defaultSite, sites: { <normalizedUrl>: { token, label, updated } } }`. URLs are normalized (https:// prefix added, trailing slash stripped) and matched by hostname so protocol/slash differences still resolve. Each tool takes an optional `site` arg; absent it, `defaultSite` is used. `WPCONNECT_SITE`/`WPCONNECT_TOKEN` env vars are a legacy fallback.
- **`_internal/`** (gitignored) holds `concept.md` (full spec & rationale), `diagnose.py`, and `wpconnect-auth-bridge.php` — reference material, not shipped code.
