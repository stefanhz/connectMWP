...
connectMWP (connectmwp.com) is a **decentralized bridge** that lets AI clients publish to self-hosted WordPress sites over MCP. The defining architectural constraint: **no central server sits in the daily traffic path.** The AI client talks straight to the user's WordPress site over HTTPS. The central web app exists only for marketing/onboarding, legal pages, and serving the plugin zip download — it is **not** in the auth or content-publishing path. This is a deliberate decision (security honeypot avoidance, $0 proxy cost, no uptime liability) — see `_internal/concept.md` §5 for the dismissed alternatives, and do not reintroduce a request-proxying server.

**Three-way client connection model (as of v2.2.0):**

| Client | Method | Setup |
|---|---|---|
| Claude Desktop/Code, Cursor, Cline | **Stdio + Ed25519 pairing** | `npx -y connectmwp-mcp add-site --enroll "…"` (one-time per site) |
| Antigravity, Gemini CLI, other remote clients with custom-header support | **API token (`cmwp_cgpt_` bearer, `Authorization: Bearer`)** | Generate token in WP Admin → Settings → connectMWP API-token card; paste into client alongside connector URL |
| ChatGPT | **Plugin-hosted OAuth 2.1** | Add connector in ChatGPT with `/mcp` URL + Auth=OAuth; sign in as admin; approve on consent screen. No token generated/pasted. |

ChatGPT uses OAuth (not an API token) because its connector UI offers only OAuth/No-Auth/Mixed authentication modes — there is no API-key field. The plugin is its own OAuth 2.1 AS+RS (no central server). See `_internal/ARCHITECTURE.md` §4.9 for the full OAuth design.
...
# WordPress plugin — no build. Packaging is manual. The zip's top folder must be
# `connectmwp/` (= wp.org slug / installed folder), but the source dir is
# `connectmwp-agent/`, so stage it first:
...
**Central web app (`connectmwp-server`) is OUT of the daily path.** It's still in the repo because it hosts the marketing landing page, legal pages (`/legal/[slug]`), and serves the plugin zip from `/public/connectmwp.zip`. The old `/callback` page from the v1 OAuth flow may still exist in code but is no longer reachable from any v2 pairing flow. Do not put auth or content-publishing logic on the server — that violates the decentralization rule.
...
- **Cross-language constant contract — media size cap (v2.0.32+):** the 10 MB upload limit exists as TWO independent literals with NO compile-time link: `MEDIA_MAX_BYTES` / `MEDIA_MAX_MB` in `connectmwp-mcp/lib/constants.js` (client-side pre-check) and `const MEDIA_MAX_BYTES` / `MEDIA_MAX_MB` in `connectmwp-agent/connectmwp-agent.php` (server-side enforcement). **Change them together** — raising one without the other produces a split-brain limit where uploads pass the client but hard-fail at the plugin (or vice versa). Same discipline as the `PAIRING_COMMAND` triplication noted for `connectmwp-server/src/lib/constants.ts`.
- **Cross-language TOOL-SCHEMA contract (v2.1.0+):** the MCP tool surface (tool `name`s + `inputSchema` field names) is now defined in TWO places with NO compile-time link: `connectmwp-mcp/index.js` (Node — canonical for stdio clients: Claude/Cursor/Cline) and `mcp_tool_definitions()` in `connectmwp-agent/connectmwp-agent.php` (PHP — served to ChatGPT via the remote `/mcp` `tools/list`). **Adding/renaming a tool or changing an `inputSchema` requires updating BOTH** — drift means a tool works for Claude but is missing/malformed for ChatGPT (or vice versa). Same discipline as the `MEDIA_MAX_BYTES` cross-language pair above. (Deliberate asymmetry: `connectmwp_upload_media` is multipart-only and is intentionally OMITTED from the PHP `mcp_tool_definitions()` — ChatGPT cannot upload media; do not "fix" the omission by adding it.)
- **Second transport + second auth method (API-token path for Antigravity / Gemini CLI / other remote clients, v2.1.0+):** alongside the stdio Ed25519 signature path, the plugin hosts a remote MCP JSON-RPC endpoint — `POST /wp-json/connectmwp/v1/mcp` (+ a `/mcp/<token>` path-token variant for hosts that strip `Authorization`) — handling `initialize` / `tools/list` / `tools/call` / `ping`, with single `application/json` responses (no SSE). It is authenticated by a per-site bearer **token** (`cmwp_cgpt_<hex>`, stored as a sha256 hash, bound to a WP user), verified by `verify_token_request` (HTTPS-only, IP rate-limit, unbound-token guard, per-request cache, touch-on-success). `tools/call` routes through the shared `dispatch_action()` — same handlers + capability checks as REST/AJAX; authz is split out into `cap_can_*` helpers with a defense-in-depth guard in `dispatch_action()`. Token blast radius ≈ a WP Application Password for the bound user on one site; the signature path remains the stronger, replay-protected device-key channel and is UNCHANGED. Admin UX is the **API token** card (generate/list/revoke, per-site cap 20). Full design in `_internal/ARCHITECTURE.md` §4.8; operator runbook in `OPERATIONS.md` §9.
- **Third auth method (ChatGPT OAuth 2.1, v2.2.0+):** ChatGPT uses OAuth because its connector UI has no API-key field. The plugin is its own OAuth 2.1 AS+RS (no central server). Key points: (a) the authorization endpoint `/connectmwp-oauth/authorize` is a cookie-native FRONT-END page (not REST) — cookie auth works without a REST nonce, which fixes the login loop; (b) discovery docs at `/.well-known/oauth-authorization-server` and `/.well-known/oauth-protected-resource` are served by `oauth_wellknown_router()` on `parse_request`; (c) client identity = CIMD (ChatGPT metadata URL) — no DCR; (d) auth-code+PKCE S256, refresh rotation; (e) access tokens (`cmwp_oat_`) are verified in `verify_token_request`'s OAuth branch alongside `cmwp_cgpt_` tokens, then routed through the same `dispatch_action()`. `form-action` CSP in the consent page must include the client's redirect origin. Full design in `_internal/ARCHITECTURE.md` §4.9; operator runbook in `OPERATIONS.md` §10.
- **Plugin auth flow lives in a single file**, `connectmwp-agent/connectmwp-agent.php`. The signature verification (`verify_request_signature`), `rest_pre_dispatch` central filter (`central_rest_auth`), per-route `permission_callback`s, AJAX dispatch, API-token DAL + `verify_token_request`, OAuth DAL + consent endpoint, and pairing screen all live in there. Treat it as the auth boundary. The `rest_pre_dispatch` signature gate has **TWO exceptions:** `/enroll` (authorized by a single-use enrollment code) and `/mcp` + `/mcp/<token>` (authorized by `verify_token_request`, which handles both `cmwp_cgpt_` API tokens and `cmwp_oat_` OAuth access tokens — auth is NOT bypassed, it moves down a layer; the no-cache headers still fire first). The OAuth authorization endpoint (`/connectmwp-oauth/authorize`) is NOT a REST route at all — it is a front-end page intercepted via `parse_request`. **Never add a REST route with `permission_callback => '__return_true'`** outside `/enroll`; every other REST endpoint must go through `verify_request_signature` OR `verify_token_request`.
...
- **Plugin zip — only ONE of the two locations is committed to git.** The release flow builds two byte-identical copies: `connectmwp.zip` at the repo root (gitignored, local build artifact for Stefan / direct uploads), and `connectmwp-server/public/connectmwp.zip` (committed — this is what serves at `https://connectmwp.com/connectmwp.zip` for end-user downloads). The zip's internal top folder is `connectmwp/` (= the wp.org slug and the installed plugin folder), even though the source working dir is `connectmwp-agent/` — the build stages the source into a `connectmwp/` folder (see Commands). Both copies must stay sha-identical after every plugin change; only the public mirror lands in git. Implication: a `git clone` of this repo will NOT yield `connectmwp.zip` at the root — devs working from clone must either build it locally or grab the public/ copy.
