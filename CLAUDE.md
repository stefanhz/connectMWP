...
- **Second transport + second auth method (ChatGPT, v2.1.0+):** alongside the stdio Ed25519 signature path, the plugin hosts a remote MCP JSON-RPC endpoint — `POST /wp-json/connectmwp/v1/mcp` (+ a `/mcp/<token>` path-token variant for hosts that strip `Authorization`) — handling `initialize` / `tools/list` / `tools/call` / `ping`, with single `application/json` responses (no SSE). It is authenticated by a per-site bearer **token** (`cmwp_cgpt_<hex>`, stored as a sha256 hash, bound to a WP user), verified by `verify_token_request` (HTTPS-only, IP rate-limit, unbound-token guard, per-request cache, touch-on-success). `tools/call` routes through the shared `dispatch_action()` — same handlers + capability checks as REST/AJAX; authz is split out into `cap_can_*` helpers with a defense-in-depth guard in `dispatch_action()`. Token blast radius ≈ a WP Application Password for the bound user on one site; the signature path remains the stronger, replay-protected device-key channel and is UNCHANGED. Admin UX is a "Connect ChatGPT (beta)" card (generate/list/revoke, per-site cap 20). Full design in `_internal/ARCHITECTURE.md` §4.8; operator runbook in `OPERATIONS.md` §9.
- **Plugin auth flow lives in a single file**, `connectmwp-agent/connectmwp-agent.php`. The signature verification (`verify_request_signature`), `rest_pre_dispatch` central filter (`central_rest_auth`), per-route `permission_callback`s, AJAX dispatch, ChatGPT token DAL + `verify_token_request`, and pairing screen all live in there. Treat it as the auth boundary. The `rest_pre_dispatch` signature gate now has **TWO exceptions, not one:** `/enroll` (authorized by a single-use enrollment code) and `/mcp` + `/mcp/<token>` (authorized by a bearer token via the route's `verify_token_request` permission_callback — auth is NOT bypassed, it moves down a layer; the no-cache headers still fire first). **Never add a route with `permission_callback => '__return_true'`** outside `/enroll`; every other endpoint must go through `verify_request_signature` OR `verify_token_request`.
